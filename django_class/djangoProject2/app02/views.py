...
def load_file(request):
    ...
    if request.method == "GET":
        form = upform()
        return render(request, "file.html", {"form": form, "title": title})
    form = upform(data=request.POST, files=request.FILES)
    if form.is_valid():
        image_obj = form.cleaned_data.get("img")
        db_img_path = os.path.join("static/", "img/", image_obj.name)
        print(db_img_path)
        img_path = os.path.join("app1", "static", "img", image_obj.name)
        f = open(img_path, mode="wb")
        for chunk in image_obj.chunks():
            f.write(chunk)
        f.close()
        # 写入数据库
        models.Admin1.objects.create(
            username=form.cleaned_data["username"],
            password=form.cleaned_data['password'],
            img=db_img_path,
        )
        return redirect("/" + db_img_path)
    return render(request, "file.html", {"form": form, "title": title})
...
def citys(request):
    if request.method == "GET":
        quersey = models.city.objects.all()
        return render(request, "city.html", {"quersey": quersey})
