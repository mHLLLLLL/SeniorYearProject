...
class inference:
    def __init__(self, model_path):
        ...
        # 绘制相关参数
        self.kpts_shape = [13, 3]  # 关键点 shape
        self.bbox_name = 'tiger'
        ...
        print(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),self.model_path))
        self.ort_session = onnxruntime.InferenceSession(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),self.model_path),
    ...
    def run_to_onnx(self, img_bgr, x_ratio, y_ratio):
        '''
        ...
        # ONNX Runtime 推理预测
        ort_output = self.ort_session.run([self.output_name], {self.input_name: input_tensor.cpu().numpy()})[0]
        # 转 Tensor
        ...
        # 解析关键点检测预测结果
        pred_kpts = pred[:, 6:].view(len(pred), self.kpts_shape[0], self.kpts_shape[1])
        ...
        for idx in range(num_bbox):  # 遍历每个框
            ...
            bbox_label = self.bbox_name

            # 画框
            img_bgr = cv2.rectangle(img_bgr, (bbox_xyxy[0], bbox_xyxy[1]), (bbox_xyxy[2], bbox_xyxy[3]), self.bbox_color,
                                    self.bbox_thickness)

            # 写框类别文字：图片，文字字符串，文字左上角坐标，字体，字体大小，颜色，字体粗细
            img_bgr = cv2.putText(img_bgr, bbox_label,
                                  (bbox_xyxy[0] + self.bbox_labelstr['offset_x'], bbox_xyxy[1] + self.bbox_labelstr['offset_y']),
    ...
    def run_to_onnx_cam(self):
        ...
        while cap.isOpened():
            ...
            # X 方向 图像缩放比例
            x_ratio = frame.shape[1] / self.input_width
            # Y 方向 图像缩放比例
            y_ratio = frame.shape[0] / self.input_height
            ...
            ## 逐帧处理
            try:
                frame = self.run_to_onnx(frame, x_ratio, y_ratio)
            except:
                pass
    ...
    def run_to_onnx_video(self, input_path=None, weight=None, outpath=None):
    ...
    def run_to_onnx_img(self,input_path=None, outpath=None):
        # filehead = input_path.split('/')[-1]
        # output_path = "{}-out-".format(weight.split('/')[-1].split(".")[-2]) + filehead
        output_path = outpath

        result = cv2.imread(input_path)
        x_ratio = result.shape[1] / self.input_width
        # Y 方向 图像缩放比例
        y_ratio = result.shape[0] / self.input_height
        img = self.run_to_onnx(result, x_ratio, y_ratio)
